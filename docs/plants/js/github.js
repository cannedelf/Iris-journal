// Thin wrapper around the GitHub Contents API for Pink to Leaf.
// Same pattern as the meal planner and budget tracker: read and write files straight
// back to the repo, so every change is committed with full version history.
//
// The token/repo/branch keys are SHARED with the other Sunshine apps (same localStorage,
// same origin) — set the token up once and every app can auto-save. The access token
// lives only in this browser — never in the repo.
...
// e.g. https://cannedelf.github.io/iris-journal/plants/  ->  owner "cannedelf", repo "iris-journal".
function detectRepo() {
...
export const gh = {
  ...
  // Just the blob SHA of a file, or null — used to overwrite a photo without decoding it.
  async getSha(path) {
    const res = await this._api(`${path}?ref=${encodeURIComponent(this.branch)}`);
    if (res.status === 404) return null;
    if (!res.ok) throw new Error(`GitHub read failed (${res.status}): ${await res.text()}`);
